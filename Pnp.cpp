...
int main() {
    ...
    /*PNP算法反解相机坐标*/
    vector<Mat> rvecMat,tvecMat;
    for(int i=0;i<count;i++)
    {
        Mat rvecArray,tvecArray,tempTvecMat,tempRvecMat;

        solvePnP(object_points[i],image_points_seq[i],cameraMatrix,distCoeffs,rvecArray,tvecArray);

        Rodrigues(rvecArray,tempRvecMat);
        Rodrigues(tvecArray,tempTvecMat);

        rvecMat.push_back(tempRvecMat);
        tvecMat.push_back(tempTvecMat);
    }
    for(int i=0;i<rvecMat.size();i++)
    {
        cout<<"图片"+to_string(i)+"的旋转矩阵为"<<rvecMat[i]<<endl;
        cout<<"图片"+to_string(i)+"的平移矩阵为"<<tvecMat[i]<<endl;
        cout<<"----------------------------------------------------------------------------------------------------"<<endl;
    }
